...
class AdminOverviewWidget extends StatsOverviewWidget
{
    ...
    protected function getStats(): array
    {
        return [
            ...
            Stat::make('Перешли к оплате', Order::query()->where('status', 'pending')->count())
                ->description('Заказы, которые ещё ждут оплату')
                ->descriptionIcon(Heroicon::OutlinedCreditCard)
                ->color('warning'),

            Stat::make('Оплачено', Order::query()->where('status', 'paid')->count())
                ->description('Успешные оплаты по тарифам')
